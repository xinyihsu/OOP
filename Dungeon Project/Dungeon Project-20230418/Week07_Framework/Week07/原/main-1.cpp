...
//==================================================================
{
	...
	Trigger* temp = new Trigger(tempPosition.y, tempPosition.x);
	gTrigger.push_back(temp);
	gBoard[tempPosition.y][tempPosition.x] = gTrigger[0]->getIcon();

	while (!isPositionValid(tempPosition)) {
		tempPosition.y = rand() % (rowN - 2) + 1;
		tempPosition.x = rand() % (colN - 2) + 1;
	}
	Trigger* temp2 = new Trigger(tempPosition.y, tempPosition.x);
	gTrigger.push_back(temp2);
	gBoard[tempPosition.y][tempPosition.x] = gTrigger[0]->getIcon();
	...
	gCreature.setPos(tempPosition.y, tempPosition.x);
	gBoard[tempPosition.y][tempPosition.x] = gCreature.getIcon();

	while (!isPositionValid(tempPosition)) {
		tempPosition.y = rand() % (rowN - 2) + 1;
		tempPosition.x = rand() % (colN - 2) + 1;
	}
	gHero.setPos(tempPosition.y, tempPosition.x);
...
//==================================================================
{
	...
	/*Please implement your code here*/
	std::vector<std::vector<char>> drawBoard(GHEIGHT, std::vector<char>(GWIDTH, GWALL));

	for (int i = 1; i < GHEIGHT - 1; i++) {
		for (int j = 1; j < GWIDTH - 1; j++) {
			drawBoard[i][j] = GNOTHING;
		}
	}
