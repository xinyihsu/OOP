 ...
 * Description: 基本輸入方向移動功能，w s a d 移動腳色上下左右，空白改變腳色站立之地板字元，到T上可以增加經驗
			 ESC是離開畫面。同時更新圖版上的資訊。
 *************************************************************************/
#include "main.h"
#include "Hero.h"
...
//==================================================================
{
	...
	std::string input; //the input of information
	std::getline(std::cin, input);

	//judge the input type
	if (input.substr(0, 4).compare("Move") == 0) { //input move
		int MoveX = input[6] - '0';
		int MoveY = input[8] - '0';
		Position pos = gHero.getPos(); //get the position of hero

		//test the direction hero move
		if (abs(pos.x - MoveX) == 0) { //move straight
			if (pos.y - MoveY == 1) { //move up
				inputType = ValidInput::EW;
			}
			else if (pos.y - MoveY == -1) { //move down
				inputType = ValidInput::ES;
			}
		}
		else if (abs(pos.y - MoveY) == 0) { // move horizontal
			if (pos.x - MoveX == 1) { //move left
				inputType = ValidInput::EA;
			}
			else if (pos.x - MoveX == -1) { //move right
				inputType = ValidInput::ED;
			}
		}
	}
	else if (input.substr(0, 4).compare("Jump") == 0) { //the input is jump
		jumpX = input[6] - '0';
		jumpY = input[8] - '0';
		inputType = ValidInput::EJUMP;
	}
	else if (input.substr(0, 4).compare("Exit") == 0) { //input Exit
		inputType = ValidInput::EESC;
	}
	else {
		//judge the end of the input
		switch (input.back()) {
		case 'w':
		case 'W':
			inputType = ValidInput::EW;
